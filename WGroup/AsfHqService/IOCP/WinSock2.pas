...
procedure GetAcceptExSockaddrs;  external    MSWSOCK_DLL name 'GetAcceptExSockaddrs';
...
procedure SET_NETBIOS_SOCKADDR(snb: PSOCKADDR_NB; const SnbType: Word; const Name: PAnsiChar; const Port: AnsiChar);
var
  len                              : Integer;
begin
  if snb <> nil then with snb^ do begin
      ...
      if len >= NETBIOS_NAME_LENGTH - 1 then System.Move(Name^, snb_name, NETBIOS_NAME_LENGTH - 1)
        ...
        FillChar((PAnsiChar(@snb_name) + len)^, NETBIOS_NAME_LENGTH - 1 - len, ' ');
      end;
