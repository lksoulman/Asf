...
begin
  if (Length(Str) > 0) and (Ord(Str[Length(Str)]) = 0) then
    raise Exception.Create('Error: the last AnsiChar is NULL AnsiChar.');
  if Length(Key) < 8 then
    while Length(Key) < 8 do Key := Key + AnsiChar(Chr(0));
  while Length(Str) mod 8 <> 0 do Str := Str + AnsiChar(Chr(0));
  ...
  begin
    ...
    for J := 0 to 7 do
      StrResult := StrResult + AnsiChar(Chr(OutByte[J]));
  end;
...
begin
  ...
  begin
    ...
    Str := Str +  AnsiChar(Chr(HexToInt(Temp)));
  end;
