...
type
  ...
  // StatusBarUI
  TStatusBarUI = class(TCustomNCUI)
  private
    // RefreshTimer
...
begin
  Result := True;
  if (FStatusBarUI.FRenderDC.MemDC = 0)
    or (FStatusHqDataMgr = nil)
    or (FStatusBarUI.Parent.MouseMoveId = FStatusBarUI.FLStatusHqItem.Id)
    or (FStatusBarUI.Parent.MouseMoveId = FStatusBarUI.FRStatusHqItem.Id) then Exit;
  ...
  try
    for LIndex := 0 to FHqInfoQueue.GetCount - 1 do begin
      ...
      if LHqInfo <> nil then begin
        ...
        GetTextSizeX(FStatusBarUI.FRenderDC.MemDC,
...
begin
  ...
  if (FStatusNewsDataMgr = nil)
    ...
    or (FStatusBarUI.FRenderDC.MemDC = 0)
    or (FStatusBarUI.Parent.MouseMoveId = FId) then Exit;
...
begin
  ...
  try
    LOBJ := SelectObject(FStatusBarUI.FRenderDC.MemDC,
      FStatusBarUI.FAppContext.GetGdiMgr.GetFontObjHeight18);
    try
      for LIndex := 0 to FDrawNewsInfoQueue.GetCount - 1 do begin
        LNewsInfo := FDrawNewsInfoQueue.GetElement(LIndex);
      ...
      SelectObject(FStatusBarUI.FRenderDC.MemDC, LOBJ);
    end;
  finally
...
begin
  ...
  if FId = FStatusBarUI.Parent.MouseMoveId then begin
    OffsetRect(LSrcRect, 20, 0);
    if FId = FStatusBarUI.Parent.MouseDownId then begin
      OffsetRect(LSrcRect, 20, 0);
    end;
  end;
...
constructor TStatusBarUI.Create(AContext: IAppContext; AParent: TCustomMasterUI);
begin
  inherited;
  FWMPAINT := WM_NCPAINT_STATUSBAR;
  DoAddStatusItems;
...
procedure TStatusBarUI.LButtonClickComponent(AComponent: TComponentUI);
begin

end;

procedure TStatusBarUI.DoAddStatusItems;
var
  LStatusItem: TStatusItem;
begin
  LStatusItem := TStatusHqItem.Create(Self);
  LStatusItem.Id := FParent.ComponentId.GenerateId;
  TStatusHqItem(LStatusItem).SetHqType(hstLeft);
  FComponents.Add(LStatusItem);
  FComponentDic.AddOrSetValue(LStatusItem.Id, LStatusItem);
  ...
  LStatusItem.Id := FParent.ComponentId.GenerateId;
  FNewsItemIndex := FComponents.Add(LStatusItem);
  FComponentDic.AddOrSetValue(LStatusItem.Id, LStatusItem);
...
procedure TStatusBarUI.DoCalcComponentsRect;
var
  LIndex: Integer;
  LStatusItem: TStatusItem;
...
procedure TStatusBarUI.DoDrawBK(ARenderDC: TRenderDC);
begin
  FillSolidRect(ARenderDC.MemDC, @FComponentsRect, FAppContext.GetGdiMgr.GetColorRefMasterStatusBarBack);
end;

procedure TStatusBarUI.DoRefreshInvalidateTimer(Sender: TObject);
begin
